...
SQ_ORDERS AS (
  -- [INFA: SourceQualifier] SQ_ORDERS
  SELECT CUST_ID,
         ORDER_AMT,
         ORDER_DT,
         ORDER_ID,
...
EXPR_CUST_ENRICH_LOAD AS (
  ...
  SELECT
    ...
    -- Expression: :LKP.LKP_CUST_SEGMENT(CUST_ID)
    -- SF rewrite:  :LKP.LKP_CUST_SEGMENT(CUST_ID)
  -- TODO: VERIFY — MEDIUM confidence mapping
    :LKP.LKP_CUST_SEGMENT(CUST_ID) AS CUST_SEGMENT,
...
LKP_CUST_SEGMENT AS (
  -- [INFA: Lookup] LKP_CUST_SEGMENT → CUSTOMER_MASTER
  SELECT
    src.*,
    lkp.CUST_SEGMENT
  FROM   EXPR_CUST_ENRICH_LOAD src
  LEFT   JOIN PROD_DB.CORTEX_CHAT_APP.CUSTOMER_MASTER lkp
      ON lkp.CUST_ID = src.CUST_ID
),
LKP_CUST_DETAILS AS (
  -- [INFA: Lookup] LKP_CUST_DETAILS → CUSTOMER_TIER_REF
  SELECT
    src.*,
    lkp.CUST_TIER
  FROM   LKP_CUST_SEGMENT src
  LEFT   JOIN PROD_DB.CORTEX_CHAT_APP.CUSTOMER_TIER_REF lkp
      ON CUST_ID = CUST_ID AND ACTIVE_FLG = 'Y'
)
