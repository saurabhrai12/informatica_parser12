...
SQ_ALERTS AS (
  -- [INFA: SourceQualifier] SQ_ALERTS — SQ override SQL
  SELECT * FROM ALERTS_SRC WHERE ALERT_STATUS != 'VOID'
),
...
-- [INFA: Router] group condition: RISK_SCORE >= 80
...
WHERE  RISK_SCORE >= 80;
-- [INFA: Router] group condition: RISK_SCORE >= 40 AND RISK_SCORE < 80
INSERT INTO PROD_DB.CORTEX_CHAT_APP.MEDIUM_RISK_ALERTS
  (ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID)
SELECT ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID
FROM   EXPR_ALERT_ROUTER_LOAD
WHERE  RISK_SCORE >= 40 AND RISK_SCORE < 80;
-- [INFA: Router] group condition: RISK_SCORE > 0 AND RISK_SCORE < 40
INSERT INTO PROD_DB.CORTEX_CHAT_APP.LOW_RISK_ALERTS
  (ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID)
SELECT ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID
FROM   EXPR_ALERT_ROUTER_LOAD
WHERE  RISK_SCORE > 0 AND RISK_SCORE < 40;
-- [INFA: Router] group condition: TRUE
INSERT INTO PROD_DB.CORTEX_CHAT_APP.REJECTED_ALERTS
  (ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID)
SELECT ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID
FROM   EXPR_ALERT_ROUTER_LOAD
WHERE  TRUE;
-- [INFA: Router] DEFAULT group
INSERT INTO PROD_DB.CORTEX_CHAT_APP.REJECTED_ALERTS
  (ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID)
SELECT ALERT_ID, ALERT_TYPE, RISK_SCORE, CUST_ID
FROM   EXPR_ALERT_ROUTER_LOAD
WHERE  NOT (RISK_SCORE >= 80) AND NOT (RISK_SCORE >= 40 AND RISK_SCORE < 80) AND NOT (RISK_SCORE > 0 AND RISK_SCORE < 40);$$;
